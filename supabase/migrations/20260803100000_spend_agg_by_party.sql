-- Party (đối tác) aggregates + drill-down filter for analytics.
...
as $$
  with grouped as (
    select
      (
        coalesce(nullif(btrim(party_code), ''), '—')
        || ' — '
        || coalesce(nullif(btrim(party_name), ''), '—')
      ) as label,
    ...
    where payment_date is not null
      ...
      and (
        (party_code is not null and btrim(party_code) <> '')
        or (party_name is not null and btrim(party_name) <> '')
      )
    group by 1
  ),
  ordered as (
...
revoke all on function public.spend_agg_by_party(date, date, int) from public;
grant execute on function public.spend_agg_by_party(date, date, int) to authenticated;

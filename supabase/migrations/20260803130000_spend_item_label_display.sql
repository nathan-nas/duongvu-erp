-- Hàng hóa labels: show name-only (or code-only) when one side is blank — no "— — " prefix.

create or replace function public.spend_item_label(p_code text, p_name text)
returns text
language sql
immutable
as $$
...
revoke all on function public.spend_item_label(text, text) from public;
grant execute on function public.spend_item_label(text, text) to authenticated;
